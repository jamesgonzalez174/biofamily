...
ALTER TABLE public.pharmacies ADD COLUMN IF NOT EXISTS tickets integer NOT NULL DEFAULT 0;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS total_tickets integer NOT NULL DEFAULT 0;
...
BEGIN
  ...
  _ticket_share := CASE WHEN _member_count > 0 THEN floor(_tickets::numeric / _member_count)::integer ELSE 0 END;

  UPDATE public.invoices
  SET total_tickets = _tickets
  WHERE id = _inv.id;
