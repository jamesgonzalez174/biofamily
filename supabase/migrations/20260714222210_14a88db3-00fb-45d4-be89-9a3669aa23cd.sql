...
ALTER TABLE public.pharmacies
  ADD COLUMN IF NOT EXISTS invoice_references text[] NOT NULL DEFAULT '{}';

DROP FUNCTION IF EXISTS public.admin_list_pharmacies(text, integer, integer);
...
REVOKE EXECUTE ON FUNCTION public.admin_list_pharmacies(text, integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_list_pharmacies(text, integer, integer) TO authenticated, service_role;
